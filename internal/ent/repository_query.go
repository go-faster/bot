...
func (_q *RepositoryQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *RepositoryQuery) QueryCommits() *GitCommitQuery {
	query := (&GitCommitClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *RepositoryQuery) Clone() *RepositoryQuery {
	...
	return &RepositoryQuery{
		...
		withCommits:      _q.withCommits.Clone(),
...
func (_q *RepositoryQuery) WithOrganization(opts ...func(*OrganizationQuery)) *RepositoryQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
...
func (_q *RepositoryQuery) WithCommits(opts ...func(*GitCommitQuery)) *RepositoryQuery {
	query := (&GitCommitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCommits = query
	return _q
}
...
func (_q *RepositoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Repository, error) {
	...
	if query := _q.withCommits; query != nil {
		if err := _q.loadCommits(ctx, query, nodes,
	...
	for name, query := range _q.withNamedCommits {
		if err := _q.loadCommits(ctx, query, nodes,
...
func (_q *RepositoryQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *Organization)) error {
...
func (_q *RepositoryQuery) loadCommits(ctx context.Context, query *GitCommitQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *GitCommit)) error {
...
func (_q *RepositoryQuery) WithNamedCommits(name string, opts ...func(*GitCommitQuery)) *RepositoryQuery {
	query := (&GitCommitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedCommits == nil {
		_q.withNamedCommits = make(map[string]*GitCommitQuery)
	}
	_q.withNamedCommits[name] = query
	return _q
}
